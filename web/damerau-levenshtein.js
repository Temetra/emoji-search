...
function DamerauLevenshtein(prices, damerau) {
    ...
    //
    // insert: function (inserted, index) { return NUMBER; }
    //
    // remove: function (removed, index) { return NUMBER; }
    //
    ...
    switch (typeof prices.insert) {
        ...
        case 'number': insert = (c, i) => prices.insert; break;
        default: insert = (c, i) => 1; break;
    }

    switch (typeof prices.remove) {
        case 'function': remove = prices.remove; break;
        case 'number': remove = (c, i) => prices.remove; break;
        default: remove = (c, i) => 1; break;
    }

    switch (typeof prices.substitute) {
        case 'function': substitute = prices.substitute; break;
        case 'number': substitute = (from, to) => prices.substitute; break;
        default: substitute = (from, to) => 1; break;
    }

    switch (typeof prices.transpose) {
        case 'function': transpose = prices.transpose; break;
        case 'number': transpose = (backward, forward) => prices.transpose; break;
        default: transpose = (backward, forward) => 1; break;
    }
        ...
        if (down === across) {
            ...
            down.forEach(function (d, i) {
                ...
                across.forEach(function (a, j) {
                    ...
                    else if (i === 0) ds[i][j] = ds[i][j - 1] + insert(a, j);
                    // Down -> empty across (j == 0) by deleting
                    else if (j === 0) ds[i][j] = ds[i - 1][j] + remove(d, j);
                    else {
